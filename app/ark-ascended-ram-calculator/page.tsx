...
// Rates derived from tier pricing with ~$0.10/GB NVMe disk subtracted
const TIERS = {
  budget: { label: "Budget", pricePerGb: 1.1 },
  premium: { label: "Premium", pricePerGb: 1.15 },
} as const;
type Tier = keyof typeof TIERS;

type ModLevel = "vanilla" | "light" | "heavy";

// ASA runs on Unreal Engine 5 — base RAM is higher than ASE
const MAPS = {
  "the-island":     { label: "The Island",     vanilla: 15, light: 17, heavy: 19 },
  "the-center":     { label: "The Center",     vanilla: 15, light: 17, heavy: 19 },
  "scorched-earth": { label: "Scorched Earth", vanilla: 13, light: 15, heavy: 17 },
  aberration:       { label: "Aberration",     vanilla: 13, light: 15, heavy: 17 },
  extinction:       { label: "Extinction",     vanilla: 15, light: 17, heavy: 19 },
  astraeos:         { label: "Astraeos",       vanilla: 21, light: 23, heavy: 26 },
} as const;
...
export default function ArkAscendedRamCalculator() {
  ...
  const price = (recommendedRam * TIERS[tier].pricePerGb).toFixed(2);
    ...
    <div className="container mx-auto max-w-7xl px-4 sm:px-6 py-8 sm:py-2 md:py-2 space-y-4 bg-transparent">
      ...
      <Alert className="calculator-info-bar rounded-lg border-[#222F45] bg-[#0E1222] text-[#A4BDDE] [&>svg]:text-[#2B7FFF] px-3 py-3 sm:px-4 sm:py-3">
        ...
        <AlertDescription className="text-[#A4BDDE] text-sm sm:text-base">
          RAM estimates are based on ARK: Survival Ascended&apos;s Unreal Engine 5 requirements. Actual usage may increase over time with player activity, builds, and tamed dinos. +0.5 GB is added per player above 10.
        </AlertDescription>
      </Alert>
    </div>
  );
